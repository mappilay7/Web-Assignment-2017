...
const server = 'mongodb://'+process.env.IP+':27017/'+mydatabase
console.log(server)
...
exports.addQuery = (laxmanndata, callback) => {
  console.log('addQuery()...');
  /* now we have extracted the data we can use it to create a new 'Query' object that adopts the correct schema. */
  const newQuery = new Query({ query: laxmanndata.query, count: laxmanndata.data.length, results:JSON.stringify(laxmanndata.data)  });
...
exports.addMovie = (laxmanndata, callback) => {
  console.log('addMovie()...');
  laxmanndata.like = 0;
  laxmanndata.dislike = 0;
  var newMovie = new Movie({ id: laxmanndata.id, title: laxmanndata.title, original_title: laxmanndata.original_title,
                 status: laxmanndata.status, revenue: laxmanndata.revenue, overview: laxmanndata.overview, original_language: laxmanndata.original_language,
                 poster_path: laxmanndata.poster_path, release_date:laxmanndata.release_date
  });
